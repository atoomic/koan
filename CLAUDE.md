...
All queue mutations go through `locked_store()` in `mission_store.py`. See
`docs/architecture/mission-lifecycle.md` for the full lifecycle diagram and
transition reference.

### Key modules (`koan/app/`)

**Core data & config:**
- **`mission_store.py`** — Structured mission store (JSON source of truth). All queue mutations go through `locked_store()` context manager. `missions.md` is a generated view. See `docs/architecture/mission-lifecycle.md`.
