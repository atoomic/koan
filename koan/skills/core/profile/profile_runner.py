"""
Koan -- Performance profile runner.

Performs a read-only performance analysis of a project codebase and saves
the report to the project's learnings directory. Optionally queues
top findings as missions.

Pipeline:
1. Build a performance profile prompt with project context
...
CLI:
    python3 -m skills.core.profile.profile_runner \
...
def build_profile_prompt(
    ...
    """Build a prompt for Claude to perform performance profiling."""
    ...
    if pr_url:
        prompt += (
            ...
            f"Use `gh pr diff` or read the changed files to understand "
            f"what was modified, then assess the performance impact of "
            f"those specific changes."
        )
    return prompt
...
def _extract_report_body(raw_output: str) -> str:
    ...
    match = re.search(r'(Performance Profile\b.*)', raw_output, re.DOTALL)
    if match:
...
def _extract_perf_score(report: str) -> Optional[int]:
    """Extract the performance score from the report.

    Returns the score as an integer (1-10) or None if not found.
    """
    match = re.search(r'\*\*Performance Score\*\*:\s*(\d+)/10', report)
    if match:
...
def _save_report(
    ...
    """Save the profile report to the project's learnings directory."""
    from datetime import datetime as _dt

    learnings_dir = instance_dir / "memory" / "projects" / project_name
    learnings_dir.mkdir(parents=True, exist_ok=True)

    report_path = learnings_dir / "performance_profile.md"

    timestamp = _dt.now().strftime("%Y-%m-%d %H:%M")
    header = f"<!-- Last scan: {timestamp} -->\n"
    if perf_score is not None:
        header += f"<!-- Performance score: {perf_score}/10 -->\n"
    header += "\n"
...
def run_profile(
    ...
    """Execute a performance profile analysis on a project.

    Args:
        project_path: Local path to the project.
        project_name: Project name for labeling.
        instance_dir: Path to instance directory.
        pr_url: Optional PR URL to focus the analysis on.
        notify_fn: Optional callback for progress notifications.
        skill_dir: Optional path to the profile skill directory for prompts.
    ...
    # Step 1: Build prompt
    context = f" (PR: {pr_url})" if pr_url else ""
    notify_fn(f"\U0001f50d Profiling {project_name}{context}...")
    prompt = build_profile_prompt(
        project_name, pr_url=pr_url, skill_dir=skill_dir,
    )
    ...
    try:
        ...
        return False, f"Profile scan failed: {e}"

    if not raw_output:
        return False, f"Profile scan produced no output for {project_name}."

    # Step 3: Extract structured report
    report = _extract_report_body(raw_output)
    perf_score = _extract_perf_score(report)

    # Step 4: Save report
    report_path = _save_report(instance_path, project_name, report, perf_score)
    ...
    # Build summary
    score_text = f" (score: {perf_score}/10)" if perf_score is not None else ""
    queue_text = f", {queued} missions queued" if queued else ""
    summary = (
        f"Performance profile saved to {report_path.name}{score_text}{queue_text}"
    )
    notify_fn(f"\u2705 {summary}")
...
def main(argv=None):
    """CLI entry point for profile_runner."""
    import argparse

    parser = argparse.ArgumentParser(
        description="Profile a project for performance issues."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--pr-url",
        help="Optional PR URL to focus the analysis on",
    )
    parser.add_argument(
