...
def main_loop():
    ...
    try:
        ...
        while True:
            # --- Stop check ---
            # Keep the critical section tiny: only the existence-check + unlink
            # run under the lock. The slow Telegram notify happens afterward so
            # concurrent signallers don't serialize on an HTTP round-trip.
            ...
            # Only the existence-check + unlink run under the lock; the 120s+
            # git pull in _handle_update() runs outside it so concurrent
            # signallers don't serialize on the update.
