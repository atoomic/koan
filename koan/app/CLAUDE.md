...
- `pending-jira-plan-publishes/` — Jira `/plan` comments generated but not yet confirmed delivered (`jira_plan_publish.py`); replayed on the next `/plan` for the same issue so a failed publish never costs a second model run, and dropped after three failed runs or on expiry. A stage that cannot be parsed is set aside as `<digest>.corrupt` and audited rather than silently discarded
