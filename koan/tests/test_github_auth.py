...
class TestSetupGithubAuth:
    ...
    def test_returns_false_even_if_alert_fails(self, mock_token, monkeypatch):
        monkeypatch.setenv("GITHUB_USER", "my-bot")
        monkeypatch.delenv("GH_TOKEN", raising=False)
        # Mock send_message to raise — imported locally so patch at source
        with patch("app.notify.send_message", side_effect=Exception("network error")):
            assert setup_github_auth() is False
...
class TestCLIEntryPoint:
    ...
    def test_exits_1_on_failure(self, monkeypatch):
        ...
        monkeypatch.setattr(subprocess, "run", mock_run)
        with patch("app.notify.send_message"):
            with pytest.raises(SystemExit) as exc_info:
                import runpy
                runpy.run_module("app.github_auth", run_name="__main__")
