...
from app.notify import send_telegram, send_message, format_and_send


class TestSendTelegram:
    """Tests for send_telegram (now delegates to messaging_provider)."""

    def setup_method(self):
        from app.messaging_provider import reset_provider
        reset_provider()
    ...
    @patch("app.messaging_provider.requests.post", side_effect=requests.RequestException("network error"))
    def test_network_error(self, mock_post):
        assert send_telegram("test") is False

    @patch("app.messaging_provider.requests.post", side_effect=ValueError("bad json"))
    ...
    def test_send_message_is_alias(self):
        """send_telegram and send_message should be the same function."""
        assert send_telegram is send_message
...
class TestFormatAndSend:
    ...
    @patch("app.notify.send_message", return_value=True)
    ...
    def test_no_koan_root_sends_fallback(self, mock_dotenv, mock_send, monkeypatch):
        ...
        assert result is True
        # Should have called send_message with some version of the message
...
class TestNotifyCLI:
    ...
    def test_cli_send_message(self, monkeypatch):
        ...
        with patch("app.messaging_provider.requests.post") as mock_post, \
    ...
    def test_cli_failure_exit_code(self, monkeypatch):
        ...
        with patch("app.notify.send_message", return_value=False), \
             pytest.raises(SystemExit) as exc_info:
            run_module("app.notify", run_name="__main__")
        assert exc_info.value.code == 1
