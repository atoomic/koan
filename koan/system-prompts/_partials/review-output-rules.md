...
- **comment**: Detailed explanation with suggested fix. Use markdown for readability: separate distinct thoughts into short paragraphs (blank line between them) and use `-` bullet points when listing more than one item. Avoid one dense block of text.
...
- **summary**: Final assessment — what's good, what needs fixing, merge readiness. Format for readability, not as a single dense paragraph: lead with a one-line verdict (the TL;DR), then a blank line, then a short `-` bullet list of the key points (one bullet per distinct issue or strength). Markdown is rendered, so use `\n\n` between blocks and `\n` between bullets. A reader should be able to skim the bullets and grasp every point without re-reading.
